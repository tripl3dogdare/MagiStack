...
#   $: Pop a and discard
#   ~: Reverse stack (i.e. [3,2,1] -> [1,2,3])
#   ;: Move bottom of stack to top (i.e. [3,2,1] -> [2,1,3])
#   .: Pop a, output as integer
#   ,: Pop a, output as ASCII character
#   =: Pop a and b, skip next command if a!=b
#   #: Skip to next #, |, or end of program
#   @: Skip to prev @, |, or start of program
#   >: Skip to final | or end of program
#   <: Skip to first | or start of program
...
#   ": Enter string mode; pushes each character's ASCII value until the next "
#   {: Push the ASCII values of each digit in a number (i.e. 15 -> [49,53])
#   _: Exit program
#
...
def main(prog):
    ...
    while p < len(prog)-1:
        ...
        if   c.isdigit(): stack.append(int(c)) # Digits
        ...
        elif c == ">": # Skip to end/last |
            p = prog.rfind("|") if prog.rfind("|") > -1 else len(prog)-1
        elif c == "<": # Skip to start/first |
            p = prog.find("|")
            ...
            p += 1
            while p < len(prog) and prog[p] != "\"":
                stack.append(ord(prog[p]))
                p += 1
        elif c == "{": # Push ASCII codes for number
            for i in str(stack.pop()): stack.append(ord(i))
